...
exports.sendRequest = async function(myDid, theirDid, proofRequestId, otherProofRequest) {
    ...
    if(proofRequestId === "proofRequestOther") {
        console.log(otherProofRequest)
        console.log("test4")
        proofRequest = otherProofRequest;
        console.log("test1")
    } else {
        await exports.getProofRequests(); // loads data into proofRequests if not already there.
        console.log(proofRequestId)
...
exports.acceptRequest = async function(messageId) {
   ...
   let crypto = await indy.crypto.sendAnonCryptedMessage(theirEndpointDid, await indy.crypto.buildAuthcryptedMessage(pairwise.my_did, message.message.origin, MESSAGE_TYPES.PROOF, proof));
    return crypto
};
